...
describe("#documents.list", () => {
  ...
  it("should allow changing sort direction", async () => {
    ...
    const anotherDoc = await (0, _factories.buildDocument)({
...
describe("#documents.search", () => {
  ...
  it("should return results in ranked order", async () => {
    ...
    const firstResult = await (0, _factories.buildDocument)({
      title: "search term",
      text: "random text",
      userId: user.id,
      teamId: user.teamId
    });
    const secondResult = await (0, _factories.buildDocument)({
      title: "random text",
      text: "search term",
      userId: user.id,
      teamId: user.teamId
...
describe("#documents.restore", () => {
  ...
  it("should restore the document to a previous version", async () => {
    ...
    const revisionId = revision.id; // update the document contents
  ...
  it("should not allow restoring a revision in another document", async () => {
    ...
    const anotherDoc = await (0, _factories.buildDocument)();
    const revision = await _models.Revision.createFromDocument(anotherDoc);
...
describe("#documents.create", () => {
  it("should create as a new document", async () => {
    ...
    const body = await res.json();
    const newDocument = await _models.Document.findByPk(body.data.id);
    expect(res.status).toEqual(200);
  ...
  it("should not allow very long titles", async () => {
    ...
    const res = await server.post("/api/documents.create", {
      body: {
        token: user.getJwtToken(),
        collectionId: collection.id,
        title: "This is a really long title that is not acceptable to Outline because it is so ridiculously long that we need to have a limit somewhere",
        text: " "
      }
    });
    expect(res.status).toEqual(400);
...
describe("#documents.delete", () => {
  ...
  it("should allow deleting document without collection", async () => {
    const {
      ...
      document,
      collection
    } = await (0, _support.seed)(); // delete collection without hooks to trigger document deletion
