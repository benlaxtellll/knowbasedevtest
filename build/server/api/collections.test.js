...
describe("#collections.update", () => {
  ...
  it("allows editing from non-private to private collection", async () => {
    ...
    expect(res.status).toEqual(200);
    expect(body.data.name).toBe("Test");
    expect(body.data.private).toBe(true); // ensure we return with a write level policy
  ...
  it("allows editing from private to non-private collection", async () => {
    ...
    expect(res.status).toEqual(200);
    expect(body.data.name).toBe("Test");
    expect(body.data.private).toBe(false); // ensure we return with a write level policy
...
describe("#collections.delete", () => {
  ...
  it("should delete collection", async () => {
    ...
    await _models.Collection.create({
      name: "Blah",
      urlId: "blah",
      teamId: user.teamId,
      creatorId: user.id
