...
describe("#groups.update", () => {
  ...
  describe("when user is admin", () => {
    let user, group;
    beforeEach(async () => {
      user = await (0, _factories.buildUser)({
        isAdmin: true
      });
      group = await (0, _factories.buildGroup)({
        teamId: user.teamId
      });
    });
...
describe("#groups.list", () => {
  ...
  it("should return groups when membership user is deleted", async () => {
    const me = await (0, _factories.buildUser)();
    const user = await (0, _factories.buildUser)({
      teamId: me.teamId
    });
    const group = await (0, _factories.buildGroup)({
...
describe("#groups.memberships", () => {
  ...
  it("should allow filtering members in group by name", async () => {
    ...
    const user3 = await (0, _factories.buildUser)({
      teamId: user.teamId,
      name: "Deleted"
    });
    const group = await (0, _factories.buildGroup)({
