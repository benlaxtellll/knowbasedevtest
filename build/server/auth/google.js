...
const client = new _googleAuthLibrary.OAuth2Client(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, `${process.env.URL}/auth/google.callback`);
const allowedDomainsEnv = process.env.GOOGLE_ALLOWED_DOMAINS; // start the oauth process and redirect user to Google
...
router.get("google.callback", (0, _authentication.default)({
  required: false
}), async ctx => {
  const {
    code
  ...
  const teamName = (0, _lodash.capitalize)(hostname); // attempt to get logo from Clearbit API. If one doesn't exist then
